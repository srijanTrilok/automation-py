...
q = 'select a.*, b.* ' \
    'from machine_lookup a, "ISS_usage_2012_q1" b ' \
    'where a.machine_type = b.machine_name_matched;'
...
for index, row in FE.iterrows():
    if row.recommendation == 'downgrade' and row.ordinal > 1:
        FE = FE.set_value(index=index, col='downgrade_to',
                     value=FE.loc[FE.ordinal == (row.ordinal - 1)]['machine_name_matched'].unique()[0])

    if row.recommendation == 'upgrade' and row.ordinal < FE.ordinal.max():
        FE = FE.set_value(index=index, col='upgrade_to',
                     value=FE.loc[FE.ordinal == (row.ordinal + 1)]['machine_name_matched'].unique()[0])
